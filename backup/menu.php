...
if($_GET["id"])
{
	...
	if($_GET["id"]!=$_SESSION["id"])
	{
		...
		$result=mysqli_query($link, "DELETE FROM Users WHERE id=".(int)$_GET["id"]);
	}else{
...
<div class="listUsers">
	...
	echo "<table>";
		...
		echo "<tbody>";
			...
			$result=mysqli_query($link, "SELECT * FROM Users LIMIT ".($numeroRegistros*($_GET["pagina"]-1)).", ".$numeroRegistros);
			while($row=mysqli_fetch_array($result, MYSQLI_ASSOC))
			{
				echo "<tr>";
					echo "<td><a href='menu.php?id=".$row["id"]."'>Eliminar</a></td>";
